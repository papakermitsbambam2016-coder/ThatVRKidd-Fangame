...
func _physics_process(_delta: float) -> void:
	var trigger_pressed := right_controller.is_button_pressed("trigger_click")
...
func create_panel() -> void:
	...
	background_material.albedo_color = Color(0.025, 0.03, 0.05, 0.96)
	...
	status_label.position = Vector3(0, -0.285, 0.025)
	status_label.pixel_size = 0.0013
...
func press_pointed_button()() -> void:
	...
	var ray_end := ray_start + - right_controller.global_basis.z * 4.0
	...
	if collider.has_meta("menu_action"):
		run_action(str(collider.get_meta("menu_action")))
...
func run_action(action: String) -> void:
	match action:
		...
		"mosa":
			...
			if mosa_enabled:
				player.push_strength = 1.15
				player.maximum_speed = 8.5
			else:
				player.push_strength = 1.0
				player.maximum_speed = 7.0
		...
		"speed":
			...
			if speed_boost_enabled:
				player.maximum_speed = 12.0
				player.push_strength = 1.4
			else:
				player.maximum_speed = 7.0
				player.push_strength = 1.0
